...
const AddProduct = () => {
  const [productData, setProductData] =
    useState<zodSchemas.ProductValidationFormType>({
      name: "",
      description: "",
      category: "",
      subcategory: "",
  ...
  const [dimensions, setDimensions] =
    useState<zodSchemas.DimensionsValidationType>({
      width: -1,
      height: -1,
      length: -1,
    });

  const [dimensionsError, setDimensionsError] =
    useState<zodSchemas.DimensionsValidationType>({
      width: -1,
      height: -1,
      length: -1,
  ...
  const [variantsError, setVariantsError] =
    useState<zodSchemas.VariantsVaidationType>({
      color: "",
      size: "",
      sku: -1,
      price: -1,
  ...
  const HandleChange = (
    ...
    if (id.startsWith("dimensions")) {
      setDimensions({
        ...dimensions,
        [id.split(" ")[1]]: value ? parseFloat(value) : value,
      });
      ...
      setProductData({
        ...productData,
        [id]:
          !["weight", "totalStock"].includes(id) || !value
            ? value
            : parseFloat(value),
      });
    }
  ...
  const SubmitForm = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const validationResult =
      zodSchemas.ProductValidationForm.safeParse(productData);
    const dimensionResult =
      zodSchemas.DimensionsValidation.safeParse(dimensions);
    ...
    if (!variantsResult.success) {
      ...
      setVariantsError({
        ...
        size: fieldErrors.size ? fieldErrors.size[0] : "",
    ...
    try {
      const cleanedProductData = {
        ...
        variants: { ...variants, images: variants.images.split(",") },
        dimensions,
      };
      const serverResponse = await baseAPI.post(
  ...
  return (
    <div className="w-full bg-[#000000] p-5 ">
      <p className="text-white  text-2xl mt-4 mb-8 text-center border-b py-5">
        ADD PRODUCT
      </p>
      ...
      <form action="" className="flex flex-col gap-5 mt-5">
        <div className="flex flex-col sm:flex-row justify-between w-full gap-5">
          <div className="flex flex-col gap-5">
            ...
            {zodError.name && (
              <span className="text-amber-600 text-xs">{zodError.name}</span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {zodError.brand && (
              <span className="text-amber-600 text-xs">{zodError.brand}</span>
            )}
          </div>
        </div>
        ...
        {zodError.totalStock === -2 && (
          <span className="text-amber-600 text-xs">
            Total stock is required and can&apos;t be negative
          </span>
        )}
        ...
        <div className="my-5 flex flex-col gap-5">
          ...
          <div className="flex flex-col sm:flex-row justify-between w-full gap-5">
            <div className="flex flex-col gap-5">
              ...
              {variantsError.price === -2 && (
                <span className="text-amber-600 text-xs">
                  Price is required and can only be +ve value
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              {variantsError.stock && variantsError.stock === -2 && (
                <span className="text-amber-600 text-xs">
                  Stock is required and can only be +ve value
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between w-full gap-5">
            <div className="flex flex-col gap-5">
              ...
              {variantsError.sku && variantsError.sku === -2 && (
                <span className="text-amber-600 text-xs">
                  SKU is required and can only be +ve value
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex justify-between items-center w-full gap-5">
                ...
                <p className="text-white text-xs">
                  {variants.color ? variants.color : "NA"}
                </p>
              </div>
        ...
        <div className="mb-5 flex flex-col gap-5">
          ...
          <div className="flex flex-col sm:flex-row justify-between w-full gap-5">
            <div className="flex flex-col gap-5">
              ...
              {dimensionsError.height === -2 && (
                <span className="text-amber-600 text-xs">
                  Height is required and can only be +ve value
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              {dimensionsError.length === -2 && (
                <span className="text-amber-600 text-xs">
                  Length is required and can only be +ve value
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between w-full gap-5">
            <div className="flex flex-col gap-5">
              ...
              {dimensionsError.width && dimensionsError.width === -2 && (
                <span className="text-amber-600 text-xs">
                  Width is required and can only be +ve value
                </span>
              )}
            </div>
        ...
        {zodError.description && (
          <span className="text-amber-600 text-xs">{zodError.description}</span>
        )}

        <label htmlFor="name" className="text-white text-xs mt-5">
          Product Tags
        </label>
        <textarea
          id="tags"
        ...
        {zodError.tags && (
          <span className="text-amber-600 text-xs">{zodError.tags}</span>
        )}
        ...
        <div className="flex flex-col sm:flex-row justify-between w-full gap-5 mt-5">
          <div className="flex flex-col gap-5">
            ...
            {zodError.weight === -1 && (
              <span className="text-amber-600 text-xs">
                Weight is required field and can only be +ve value
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {zodError.status && (
              <span className="text-amber-600 text-xs">{zodError.status}</span>
            )}
          </div>
        </div>
