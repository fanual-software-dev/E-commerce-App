...
const ProductManagement = () => {
  ...
  const PaginationButtons = ({ num }: { num: number }) => {
    return (
      <button
        id={num + ""}
        className={`bg-[#1A1A1A] w-10 py-1 cursor-pointer rounded-md ${paginationNumber === num ? "bg-[#595959] " : "bg-[#1A1A1A]"}`}
      >
        {num}
      </button>
    );
  ...
  useEffect(() => {
    const toggler = () => {
      const clickListener = (e: MouseEvent) => {
        if (
          !e.target ||
          ((e.target as HTMLElement).id !== "opener" &&
            !(e.target as HTMLElement).closest("#form-container"))
        ) {
          setAddOpen(false);
        }
      };
    ...
    <div className="text-white py-8 px-2 xl:pl-24">
      ...
      <div className="mt-5 flex flex-col gap-5">
        <div className="w-full hidden md:grid md:grid-cols-6 bg-[#1A1A1A]  shadow-md shadow-[#1b1b1b] px-5 gap-5 mt-5">
          <p className="text-white py-5 text-xs border-[#373737] border-r-2">
            Product
          </p>
          <p className="text-white py-5 text-xs border-[#373737] border-r-2">
            Name
        ...
        <div className="flex justify-end mt-5">
          <button
            ...
            className="flex justify-center gap-1 items-center w-28 text-base bg-[#121212] hover:bg-[#1A1A1A] cursor-pointer text-green-500 md:shadow-[#303030] shadow-md rounded-md p-1 "
          >
            Add
          </button>
        </div>
        ...
        <div
          id="form-container"
          className={`w-full md:py-4 h-screen  md:w-3/5 xl:w-1/3 fixed overflow-y-scroll ${addOpen ? "block" : "hidden"} right-0 top-0 z-50  transition-all duration-500`}
        >
          <X
            onClick={() => setAddOpen(!addOpen)}
      ...
      {products && (
        <div
          ...
          className="grid grid-cols-5 md:flex md:justify-center md:flex-wrap md:gap-5 gap-y-5 mt-5"
        >
