...
const Verification = () => {
  ...
  const [verifcationNumbers, setVerifcationNumbers] = React.useState<
  ...
  const HandleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (id < "5" && value.length === 1) {
      ...
      if (nextInput) {
        const nextInputElement = document.querySelector(
  ...
  const HandleSubmit = async (
    ...
    if (!result.success) {
      ...
      setErrors({
        verificationNum: fieldErrors.verificationNum
          ? fieldErrors.verificationNum[0]
          : "",
      });
      setIsSubmitting(false);
      return;
    ...
    const serverResponse = await baseAPI.post("/api/auth/verify-otp", {
      otp: verificationNum,
      email,
    });
  ...
  return (
    <div className="flex justify-center py-5">
      <form
        action=""
        className="flex flex-col gap-5 items-center justify-center w-full sm:w-3/4 md:w-3/5 xl:w-1/2 p-5"
      >
        <h1 className="text-white text-3xl font-bold">Verification</h1>
        <p className="text-[#676665] text-sm">
        ...
        {errors.verificationNum && (
          <span className="text-amber-500 -mt-6  text-xs">
            {errors.verificationNum}
          </span>
        )}
